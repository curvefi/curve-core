...
struct Quote:
    ...
    amount_out: uint256
    pool: address
    pool_balances: DynArray[uint256, 2]
...
def get_quotes(source_token: address, destination_token: address, amount_in: uint256) -> DynArray[Quote, MAX_QUOTES]:
    ...
    for pool in pools:
        ...
        if balances[i] <= amount_in or balances[j] == 0:
            continue
                ...
                Quote(
                    {
                        ...
                        pool: pool,
                        pool_balances: [balances[i], balances[j]],
                        pool_type: pool_type
                    }
                )
            )

            # stop the loop if the dynarray of quotes is full
